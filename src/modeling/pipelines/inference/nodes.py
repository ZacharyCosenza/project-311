...
def compute_call_deltas(
    ...
    from a sensitivity sweep, see docs/delta-eda). delta_rank ranks all positive
    deltas; downstream consumers (tweet pipeline) apply their own eligibility filters.
    """
    ...
    result["baseline_tgt_calls"] = result["board_key"].map(baseline)
